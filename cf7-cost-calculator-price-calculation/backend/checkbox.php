...
{
	wpcf7_add_form_tag(
		array('checkbox_custom', 'checkbox_custom*', 'radio_custom', 'radio_custom*'),
		'wpcf7_checkbox_custom_form_tag_handler',
		array(
			'name-attr' => true,
	...
	foreach ($values as $key => $value) {
		...
		if (count($values) == $count) { // last round
			...
			if ($free_text) {
				...
				if (
					wpcf7_is_posted()
					and isset($_POST[$free_text_name]) //WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized.
				) {
...
function wpcf7_checkbox_custom_validation_filter($result, $tag)
{
	...
	$value = isset($_POST[$name]) ? (array)  wp_unslash($_POST[$name]) : array(); //phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
add_action(
	'wpcf7_admin_init',
	'wpcf7_add_tag_generator_checkbox_and_radio_custom',
	90
);
function wpcf7_add_tag_generator_checkbox_and_radio_custom()
{
	...
	if (version_compare(WPCF7_VERSION, "6.0") >= 0) {
		...
		$tag_generator->add(
			'checkbox_custom',
			__('Checkbox price', 'cf7-cost-calculator-price-calculation'),
			'wpcf7_tag_generator_checkbox_custom_2',
			array("version" => 2)
		);
		$tag_generator->add(
			'radio_custom',
			__('Radio price', 'cf7-cost-calculator-price-calculation'),
			'wpcf7_tag_generator_checkbox_custom_2',
			array("version" => 2)
		);
	} else {
		$tag_generator->add(
			'checkbox_custom',
			__('Checkbox price', 'contact-form-7-cost-calculator'),
			'wpcf7_tag_generator_checkbox_custom'
		);
		$tag_generator->add(
			'radio_custom',
			__('Radio price', 'contact-form-7-cost-calculator'),
			'wpcf7_tag_generator_checkbox_custom'
		);
	}
}
...
?>
	...
	<div class="control-box">
		...
		<fieldset>
			...
			echo sprintf(
				'<span %1$s>%2$s</span>',
				wpcf7_format_atts(array( //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
...
?>
	<div class="control-box">
		<fieldset>
			<legend><?php echo sprintf(esc_html($description), $desc_link); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
					?></legend>
					...
					<tr>
						...
						<td>
							<fieldset>
								...
								<label for="<?php echo esc_attr($args['content'] . '-values'); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
											?>"><span class="description">
										<?php esc_html_e("One option per line (number|text): Ex: <strong>10|Blue $10</strong>", 'contact-form-7-cost-calculator'); ?></span></label><br />
	...
	<div class="insert-box">
		...
		<br class="clear" />
		<?php /* Translators: 1: URL, 2: Link text */ ?>
