...
{
	wpcf7_add_form_tag(
		array('select_custom', 'select_custom*'),
...
function wpcf7_select_validation_filter_custom($result, $tag)
{
	...
	if (isset($_POST[$name]) && is_array($_POST[$name])) {
		foreach (wp_unslash($_POST[$name]) as $key => $value) { //phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			if ('' === $value) {
				unset($_POST[$name][sanitize_text_field($key)]);
			}
		}
	}
...
function wpcf7_add_tag_generator_menu_custom()
{
	...
	if (version_compare(WPCF7_VERSION, "6.0") >= 0) {
		$tag_generator->add(
			'select_custom',
			__('drop-down menu price', 'cf7-cost-calculator-price-calculation'),
			'wpcf7_tag_generator_menu_custom_2',
			array("version" => 2)
		);
	} else {
		$tag_generator->add(
