(function($) {
    ...
    $( document ).ready( function () { 
    	   var loading = 0;
			...
			$("body").on("click",".yeeaddons-notice-warning-remove .notice-dismiss",function(e){
				var remove = {
					'action': 'yeeaddons_remove_notice',
					'plugin_id': $(this).closest(".yeeaddons-notice-warning-remove").data("id"),
				};
				jQuery.post(ajaxurl, remove, function(response) {
				});
    		})
			$("body").on("click",".yeeaddons-notice-warning-remove-review",function(e){
				var remove = {
					'action': 'yeeaddons_remove_notice_review',
